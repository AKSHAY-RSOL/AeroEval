  ...
  try {
    ...
    const inputJson = {
      ...
      aerodynamic_overrides: {
        ...
        propeller_class: (formData.get('propeller_class') as string) || 'SF',
      ...
      structural_overrides: {
        ...
        num_blades: formData.get('num_blades') ? Number(formData.get('num_blades')) : -1.0,
    ...
    const jsonPath = path.join(process.cwd(), '..', 'data', `user_input_${requestId}.json`);
    ...
    const outputPath = path.join(process.cwd(), '..', 'data', `sizing_output_${requestId}.json`);
    ...
    const executeEngineRun = async (v: number) => {
      ...
      try {
        const result = await execAsync(`"${exePath}" "${jsonPath}" "${outputPath}"`, {
          cwd: engineCwd,
    ...
    try {
      if (isKmode) {
        ...
        for (const v of coarseSpeeds) {
          ...
          if (!res || !res.success || !res.data || !res.data.success) {
            if (res && res.data && res.data.failures) {
              lastFailureList = res.data.failures;
            } else if (res && res.stderr) {
              lastFailureList = [{ run: 'System', stage: 'Sizing Sweep', reason: res.stderr }];
            }
            continue;
          }
        ...
        if (!outputJson || !outputJson.success) {
          return {
            success: false,
            failures: lastFailureList.length > 0 ? lastFailureList : [{ run: 'System', stage: 'Sizing Sweep', reason: 'Failed to converge on any cruising speed in the sweep.' }],
            rawOutput: outStr
          };
        }
      ...
      try { await unlink(jsonPath); } catch (_) {}
      try { await unlink(outputPath); } catch (_) {}
    }
    ...
    const formatRes = (res: any) => {
      if (!res || res.valid === false) return null;
      return {
        ...
        motor_mass_kg: res.motor_mass_kg !== undefined && res.motor_mass_kg !== null ? (typeof res.motor_mass_kg === 'number' ? res.motor_mass_kg : parseFloat(res.motor_mass_kg)) : undefined,
        wiring_mass_kg: res.wiring_mass_kg !== undefined && res.wiring_mass_kg !== null ? (typeof res.wiring_mass_kg === 'number' ? res.wiring_mass_kg : parseFloat(res.wiring_mass_kg)) : undefined,
        esc_mass_kg: res.esc_mass_kg !== undefined && res.esc_mass_kg !== null ? (typeof res.esc_mass_kg === 'number' ? res.esc_mass_kg : parseFloat(res.esc_mass_kg)) : undefined,
        complexity_mass_kg: res.complexity_mass_kg !== undefined && res.complexity_mass_kg !== null ? (typeof res.complexity_mass_kg === 'number' ? res.complexity_mass_kg : parseFloat(res.complexity_mass_kg)) : undefined,
        avionics_mass_kg: res.avionics_mass_kg !== undefined && res.avionics_mass_kg !== null ? (typeof res.avionics_mass_kg === 'number' ? res.avionics_mass_kg : parseFloat(res.avionics_mass_kg)) : undefined,
        role_aux_mass_kg: res.role_aux_mass_kg !== undefined && res.role_aux_mass_kg !== null ? (typeof res.role_aux_mass_kg === 'number' ? res.role_aux_mass_kg : parseFloat(res.role_aux_mass_kg)) : undefined
      };
    };
