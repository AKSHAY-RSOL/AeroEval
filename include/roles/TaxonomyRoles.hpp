...
public:
  ...
  double getMaxInducedVelocityMs() const override {
    // C7: agricultural downwash cap defaults to 8.0 m/s (crop-damage / drift limit).
    // Exceeding it is a soft advisory (warning), not a hard feasibility failure.
    return config.ag_max_downwash_ms > 0 ? config.ag_max_downwash_ms : 8.0;
  }
