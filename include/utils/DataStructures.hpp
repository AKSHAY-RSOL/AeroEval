...
namespace EngineData {
    ...
    struct AerodynamicOverrides {
        ...
        std::string aero_body_class = "";       // Body streamlining class override
        double c_oss_farads = -1.0;             // ESC output capacitance override (F)
        double r_ds_on_ohms = -1.0;             // ESC on-resistance override (Ohms)
        double r_i_base_ohms = -1.0;            // Battery base internal resistance override (Ohms)
    };
    ...
    struct UserInput {
        ...
        int battery_pack_count = 1;              // Number of parallel battery packs (locked capacity is treated as total terminal capacity)
