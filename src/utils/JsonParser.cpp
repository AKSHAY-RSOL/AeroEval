...
namespace Utils {
    ...
    EngineData::UserInput JsonParser::parseUserInput(const std::string& filepath) {
        ...
        input.aero_overrides.aero_body_class = "";
        input.aero_overrides.c_oss_farads = -1.0;
        input.aero_overrides.r_ds_on_ohms = -1.0;
        input.aero_overrides.r_i_base_ohms = -1.0;
        ...
        if (!aero_ovr_json.is_null()) {
            ...
            input.aero_overrides.aero_body_class = aero_ovr_json.value("aero_body_class", "");
            input.aero_overrides.c_oss_farads = aero_ovr_json.value("c_oss_farads", -1.0);
            input.aero_overrides.r_ds_on_ohms = aero_ovr_json.value("r_ds_on_ohms", -1.0);
            input.aero_overrides.r_i_base_ohms = aero_ovr_json.value("r_i_base_ohms", -1.0);
    ...
    EngineData::UserLocks JsonParser::parseUserLocks(const std::string& filepath) {
        ...
        if (!locks_json.is_null()) {
            ...
            // Check for system voltage locking or if battery is locked but cell count wasn't explicitly set in the battery block
            if (locks_json.value("is_voltage_locked", false) || (locks.is_battery_locked && locks.locked_cell_count == 0)) {
                int series_cells = locks_json.value("locked_series_cells", 0);
                if (series_cells > 0) {
                    locks.locked_cell_count = series_cells;
                }
            }
        ...
        // ── Top-level shorthand locks (flat key convenience API) ──────────────────
        // Users can also specify lock parameters directly in the root JSON object
        // without nesting under a "locks" key. These top-level values take effect
        // only if the corresponding lock was not already set by the locks block above.
        // This allows simple inputs like: {"locked_cell_count": 6, "locked_capacity_mah": 5000}
        // without requiring: {"locks": {"is_battery_locked": true, "locked_capacity_mah": 5000, ...}}
        {
            // Capacity lock (shorthand): sets both capacity AND cell count if provided
            double top_cap = j.value("locked_capacity_mah", 0.0);
            if (top_cap > 0.0 && locks.locked_capacity_mah <= 0.0) {
                locks.locked_capacity_mah = top_cap;
                locks.is_battery_locked = true;
                // Also pick up cell count from shorthand if provided alongside capacity
                int top_s = j.value("locked_cell_count", 0);
                if (top_s > 0 && locks.locked_cell_count == 0) {
                    locks.locked_cell_count = top_s;
                }
            }

            // Cell count lock (shorthand, voltage-only lock, no capacity constraint)
            {
                int top_s = j.value("locked_cell_count", 0);
                if (top_s > 0 && locks.locked_cell_count == 0) {
                    locks.locked_cell_count = top_s;
                    // Don't set is_battery_locked — only voltage is constrained
                }
            }

            // Prop diameter lock (shorthand)
            double top_d = j.value("locked_prop_diameter_in", j.value("locked_diameter_in", 0.0));
            if (top_d > 0.0 && locks.locked_diameter_in <= 0.0) {
                locks.locked_diameter_in = top_d;
                locks.is_prop_locked = true;
                double top_p = j.value("locked_prop_pitch_in", j.value("locked_pitch_in", 0.0));
                locks.locked_pitch_in = top_p;
            }

            // Motor KV lock (shorthand)
            double top_kv = j.value("locked_kv", j.value("locked_motor_kv", 0.0));
            if (top_kv > 0.0 && locks.locked_kv <= 0.0) {
                locks.locked_kv = top_kv;
                locks.is_motor_locked = true;
            }

            // Auto-reduce flight time (shorthand from root level)
            if (j.contains("auto_reduce_flight_time") && (locks_json.is_null() || !locks_json.contains("auto_reduce_flight_time"))) {
                locks.auto_reduce_flight_time = j.value("auto_reduce_flight_time", true);
            }
        }
