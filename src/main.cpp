...
void writeSizingOutput(const std::string &filepath,
  ...
  const std::string& rt = input.payload_role.type;
  if (rt != "dead_weight" && rt != "delivery" && rt != "agriculture" && rt != "agri_spray" &&
      rt != "racing" && rt != "imaging" && rt != "cinema" && rt != "cinematography" &&
      rt != "mapping" && rt != "search" && rt != "inspection") {
    nlohmann::json f;
    f["run"] = "Input Warning";
    f["stage"] = "Role Selection";
    f["reason"] = "Unrecognized payload role type '" + rt + "'. Falling back to Cinematography/Imaging math.";
    failures.push_back(f);
  }
  if (input.aero_overrides.propulsive_efficiency > 0.0 && input.t_forward_s <= 0.0) {
    nlohmann::json f;
    f["run"] = "Input Warning";
    f["stage"] = "Aerodynamic Overrides";
    f["reason"] = "propulsive_efficiency override is specified, but forward cruise flight time is zero. Propulsive efficiency will not affect sizing results since no forward segments exist.";
    failures.push_back(f);
  }
  j["failures"] = failures;
  ...
  auto format_res_json = [&input](const RunResult &r) -> nlohmann::json {
    ...
    if (std::isfinite(r.kv_min) && std::isfinite(r.kv_max) && r.kv_min >= 0.0 && r.kv_max >= 0.0) {
      res["shopping"] =
...
int main(int argc, char* argv[]) {
  std::string input_path = "data/user_input.json";
  std::string output_path = "data/sizing_output.json";
  if (argc >= 3) {
    input_path = argv[1];
    output_path = argv[2];
  }
  ...
  try {
    ...
    EngineData::UserInput input =
        Utils::JsonParser::parseUserInput(input_path);
    EngineData::UserLocks locks =
        Utils::JsonParser::parseUserLocks(input_path);
    ...
    if (!r1.success || !r2.success) {
      ...
      writeSizingOutput(output_path, input, locks, r1, r2);
      return 1;
    }
