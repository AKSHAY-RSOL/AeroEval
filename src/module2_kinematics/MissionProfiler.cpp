...
namespace Module2 {
        ...
        : aero_overrides(input.aero_overrides)
    {
        ...
        this->drag_area_multiplier = input.payload_role.drag_area_multiplier;
    }
    ...
    double MissionEvaluator::calculateParasiteDrag(double mass, double velocity, double drag_coefficient) const {
        ...
        if (this->drag_area_multiplier > 0.0) {
            equivalent_area *= this->drag_area_multiplier;
        } else if (this->active_role != nullptr && this->active_role->getRoleType() == "mapping") {
            // Apply default 1.5x mapping role multiplier to fuselage flat-plate area if not overridden
            equivalent_area *= 1.5;
        }
