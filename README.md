...
* **Conservative Lower-Bound Design**: Sized around standard Glauert momentum theory, explicit safety factors, and COTS components, targeting a positive design margin: if the engine converges and accepts a design, it is intended to fly in practice.
...
* **Dual Design Workspaces**:
  ...
  * **Physics Workspace**: A full-fidelity editor with 57 adjustable parameters for expert aerospace designers.
...
```bash
# Provide an input configuration (copy and edit the shipped template)
cp data/user_input_example.json data/user_input.json

# Run simulation; results are written to data/sizing_output.json
./build/DroneEngine
```
...
Solving for outer diameter $d_{\text{out}}$ under fatigue-adjusted allowable stress, derived transparently from the CFRP ultimate strength ($\sigma_{\text{ult}} = 720$ MPa, fatigue knockdown $f_{\text{fat}} = 0.40$, factor of safety $\text{FoS} = 1.5$):


$$
\sigma_{\text{all}} = \frac{\sigma_{\text{ult}}\,f_{\text{fat}}}{\text{FoS}} = \frac{720 \times 0.40}{1.5} = 192 \text{ MPa}
$$
...
#### Per-Class Structural Calibration
Folding-arm hinges, ruggedization hardware, and integration scale differently across vehicle categories, leaving a residual category-correlated bias in predicted empty mass. A per-class multiplier $\lambda_{\text{class}}$ is applied to the combined frame and complexity mass, fitted on the held-out calibration cohort (see *Empirical Validation*):


$$
\lambda_{\text{class}} \in \{1.50_{\text{MicroAIO}},\; 1.08_{\text{ConsumerFolding}},\; 0.95_{\text{EnterpriseRugged (quad)}},\; 0.80_{\text{EnterpriseRugged (hex/octo)}}\}
$$


#### Wiring Harness Mass (Per-Arm Current)
Copper harness mass scales with the current each conductor carries, not the pack total. Each of the $N_r$ arm runs carries only $I_{\text{max}}/N_r$ over a round-trip length $2L_{\text{arm}} = W$, so the rotor count cancels:


$$
M_{\text{wiring}} = C_{\text{wire}} \cdot \frac{I_{\text{max}}}{N_r} \cdot (N_r \cdot 2L_{\text{arm}}) \cdot S_{\text{scale}} = C_{\text{wire}}\,I_{\text{max}}\,W\,S_{\text{scale}}
$$


with $C_{\text{wire}} = 0.0003$ kg/(A·m) for racing/small platforms and $0.0015$ otherwise. Billing the full pack current over all $N_r$ arm runs over-counts copper by a factor of $N_r$ and inflates hex/octo frames super-linearly; the per-arm formulation removes this dimensional inconsistency.
...
AeroEval is validated under a strict, **non-circular calibrate/test protocol** that eliminates the circularity of fitting sizing parameters on the same data used for validation. Three structural/packaging coefficients — the per-class structural multiplier $\lambda_{\text{class}}$, the battery packaging base, and the wiring coefficient — are fitted **only** on a held-out cohort of 20 do-it-yourself (DIY) and legacy platforms, then **frozen** and evaluated **blind** on a disjoint cohort of 20 modern commercial platforms. The commercial accuracy is therefore an out-of-sample generalization result, not an in-sample fit.

* **Commercial blind-test MTOW accuracy**: **7.2% Mean Absolute Percentage Error (MAPE)** across 19 platforms spanning 377 g to 76 kg (median 6.6%; 95% CI 5.1%–9.3%).
* **Root Mean Square Error (RMSE)**: **1.59 kg**.
* **Complexity Scaling Factor $k$**: mean $k \approx 1.05 \pm 0.08$ (range 0.92–1.23), where

$$
k = \frac{\text{MTOW}_{\text{actual}}}{\text{MTOW}_{\text{predicted}}}
$$

* **Battery mass accuracy**: 7.9% MAPE on single-pack platforms. Redundant dual-battery enterprise platforms (Inspire 2/3, Matrice 300/350) show a **disclosed** ~50% battery-mass underprediction because the engine sizes for mission energy, not hot-swap pack redundancy.

> **Note on the excluded platform.** The *Agras T10 (Empty Tank Return)* configuration is a deliberately self-contradictory scenario: its pinned fixed-airframe mass plus role overhead exceeds the stated empty-return MTOW, so the solver cannot reproduce it (+62.1%, $k = 0.62$). It is excluded from the aggregate and reported separately. Including it, the full 20-platform MAPE is 9.94%.

### Commercial Blind-Test Validation Results (frozen parameters)

| Platform | Actual (kg) | Predicted (kg) | Error (%) | $k$-factor |
| :--- | :---: | :---: | :---: | :---: |
| DJI Avata 2 | 0.377 | 0.354 | -6.1% | 1.07 |
| Parrot Anafi USA | 0.485 | 0.526 | +8.5% | 0.92 |
| Custom 5-inch FPV | 0.650 | 0.609 | -6.3% | 1.07 |
| DJI Air 3 | 0.720 | 0.785 | +9.1% | 0.92 |
| DJI FPV Combo | 0.795 | 0.811 | +2.0% | 0.98 |
| DJI Mavic 3 Classic | 0.895 | 0.954 | +6.6% | 0.94 |
| DJI Mavic 3 Enterprise | 1.050 | 0.951 | -9.4% | 1.10 |
| Custom 7-inch FPV | 1.200 | 1.192 | -0.7% | 1.01 |
| DJI Phantom 4 Pro V2.0 | 1.375 | 1.192 | -13.3% | 1.15 |
| Teal Golden Eagle | 1.400 | 1.319 | -5.8% | 1.06 |
| Skydio X10 | 2.490 | 2.444 | -1.8% | 1.02 |
| DJI Inspire 3 | 3.995 | 3.571 | -10.6% | 1.12 |
| DJI Inspire 2 | 4.250 | 4.139 | -2.6% | 1.03 |
| Sony Airpeak S1 | 4.400 | 4.517 | +2.7% | 0.97 |
| DJI Matrice 300 RTK | 9.000 | 8.373 | -7.0% | 1.07 |
| DJI Matrice 350 RTK | 9.200 | 8.349 | -9.3% | 1.10 |
| Watts Innovations Prism | 20.360 | 16.505 | -18.9% | 1.23 |
| DJI FlyCart 30 | 40.000 | 35.015 | -12.5% | 1.14 |
| DJI Agras T30 | 76.000 | 73.328 | -3.5% | 1.04 |
| *Agras T10 (Empty Tank Return)* — *excluded* | 16.800 | 27.239 | +62.1% | 0.62 |

The held-out DIY/legacy calibration cohort yields 26.1% MAPE ($k \approx 1.03 \pm 0.35$); its far larger scatter reflects non-integrated, hand-assembled carbon-fiber builds and heterogeneous wiring, and is precisely why it serves as the calibration set rather than the reporting set — fitting to its variance would overstate attainable accuracy on production hardware.
...
* **Realistic Testing** (`realistic_test.py`): Runs randomized simulations across realistic payload weight classes and role envelopes to confirm correct convergence.
* **Parameter Sweeps** (`parameter_sweeps.py`): Automated sensitivity sweeps across mission and hardware parameters, reproducing the 14-parameter sensitivity summary above.

Execute the suites using:
```bash
python tests/realistic_test.py
python tests/parameter_sweeps.py
```
